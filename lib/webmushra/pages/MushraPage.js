...
MushraPage.prototype.init = function () {
  ...
  this.mushraAudioControl = new MushraAudioControl(this.audioContext, this.bufferSize, this.reference, this.conditions, this.errorHandler, this.pageConfig.createAnchor35, this.pageConfig.createAnchor70, this.pageConfig.randomize, this.pageConfig);
...
this.waveformVisualizer = new WaveformVisualizer(this.pageManager.getPageVariableName(this) + ".waveformVisualizer", tdRight, this.reference, this.pageConfig.showWaveform, this.pageConfig.enableLooping, this.mushraAudioControl);
  ...
  this.render_q(_parent,1);
  this.render_q(_parent,2);
  this.render_q(_parent,3);
  this.render_q(_parent,4);
};
...
MushraPage.prototype.btnCallbackReference = function() {
  ...
  if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
    ...
    $(".audioControlElement").text(this.pageManager.getLocalizer().getFragment(this.language, 'playButton'));
    //this.mushraAudioControl.changeSw(0);
...
MushraPage.prototype.btnCallbackCondition = function(_index) {
  ...
  if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
    ...
    $(".audioControlElement").text(this.pageManager.getLocalizer().getFragment(this.language, 'playButton'));
    this.mushraAudioControl.changeSw(_index);
...
// questionnaire
MushraPage.prototype.render_q = function (_parent, chNum) {
  var table = $("<table align='center'></table>");
  _parent.append(table);
  table.append("<tr><th>&nbsp;</th></tr><tr><th><strong>Channel " + chNum + "</strong></th></tr>");
  var i;
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
    var element = this.pageConfig.questionnaire[i];
    if(element.type === "likert") {              
      this.likert = new LikertScale(element.response, element.name + "_ch" + chNum + "_");
      var td = $("<td></td>");
...
MushraPage.prototype.save_q = function () {
  var scales = $(".scales");
  this.likerts = [];
  for (var i = 0; i  < scales.length; ++i) {// each channel
    var likert = {};
    //likert["channel"] = scales[i].name;
    for (var j = 0; j < this.pageConfig.questionnaire.length; ++j) {
      var element = this.pageConfig.questionnaire[j];
      likert[element.name] = $("input[name='"+element.name + "_ch" + (i+1) + "__response']:checked").val();
    }
    this.likerts[i] = likert;
  }
};

MushraPage.prototype.load_q = function() {
  var scales = $(".scales");
  for (var i = 0; i  < scales.length; ++i) {// each channel
    for (var j = 0; j < this.pageConfig.questionnaire.length; ++j) {
      var element = this.pageConfig.questionnaire[j];
      if ((i<this.likerts.length) && (element.name in this.likerts[i])){
        $("input[name='"+element.name + "_ch" + (i+1) + "__response'][value='"+this.likerts[i][element.name]+"']").attr("checked", "checked");
        $("input[name='"+element.name + "_ch" + (i+1) + "__response'][value='"+this.likerts[i][element.name]+"']").checkboxradio("refresh");
      }
    }
  }
};